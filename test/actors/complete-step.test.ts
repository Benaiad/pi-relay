...
describe("buildCompletionInstruction", () => {
	...
	it("includes list hint for record_list artifacts", () => {
		const listContracts = new Map<ReturnType<typeof ArtifactId>, ArtifactContract>([
			[
				ArtifactId("items"),
				{
					id: ArtifactId("items"),
...
<artifact id="notes">
  some text with leading/trailing whitespace
		...
		expect(isOk(result)).toBe(true);
		if (!isOk(result)) return;
		expect(result.value.writes.notes).toBe("some text with leading/trailing whitespace");
	});
...
line three</artifact>
	...
	it("fails when the route tag is missing", () => {
		const result = parseCompletion(`<relay-complete><artifact id="x">y</artifact></relay-complete>`);
		expect(isErr(result)).toBe(true);
	...
	it("fails when the route is empty", () => {
		const result = parseCompletion(`<relay-complete><route></route></relay-complete>`);
		expect(isErr(result)).toBe(true);
		if (!isErr(result)) return;
...
`;
	...
	it("returns an empty string when the text is only a completion tag", () => {
		expect(stripCompletionTag("<relay-complete><route>x</route></relay-complete>")).toBe("");
	});
