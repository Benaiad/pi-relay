...
import {
  actorRegistryFromDiscovery,
  discoverActors,
  formatActorList,
} from "../../src/actors/discovery.js";
...
`,
  ...
  it("loads bundled actors from bundledDir", async () => {
    ...
    const discovery = discoverActors(projectRoot, "user", {
      userDir: "/no/user/dir",
      bundledDir,
    });
...
`,
    ...
    const discovery = discoverActors(projectRoot, "both", {
      userDir: "/no/user/dir",
      bundledDir,
    });
    const worker = discovery.actors.find((a) => a.name === "worker")!;
...
`,
    ...
    expect(byName.get("scout")!.description).toBe(
      "Reconnaissance over a codebase",
    );
