...
class SimpleActorEngine implements ActorEngine {
	...
	async runAction(request: ActionRequest): Promise<ActionOutcome> {
		...
		if (!route) {
			return { kind: "no_completion", reason: "no routes", usage: fakeUsage(), transcript: [] };
		}
		return { kind: "completed", route, writes, usage: fakeUsage(), transcript: [] };
	}
}
...
describe("replay integration", () => {
	...
	it("end-to-end: discover → instantiate → compile → schedule", async () => {
		...
		const discovery = discoverPlanTemplates("/nonexistent", "user", { userDir: plansDir });
