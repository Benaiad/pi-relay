...
describe("joinNonEmpty", () => {
  it("filters out empty and false entries", () => {
    expect(joinNonEmpty(["a", "", null, "b", false, undefined, "c"])).toBe(
      "a · b · c",
    );
  });
...
describe("formatToolCall", () => {
  it("formats bash as $ command", () => {
    expect(formatToolCall("bash", { command: "npm test" }, bareTheme)).toBe(
      "$ npm test",
    );
  });
  ...
  it("formats read with path and offset/limit", () => {
    expect(
      formatToolCall(
        "read",
        { path: "/tmp/foo.ts", offset: 10, limit: 5 },
        bareTheme,
      ),
    ).toBe("read /tmp/foo.ts:10-14");
  });

  it("formats read with file_path alias", () => {
    expect(
      formatToolCall("read", { file_path: "/tmp/foo.ts" }, bareTheme),
    ).toBe("read /tmp/foo.ts");
  });
  ...
  it("formats write with line count", () => {
    ...
    ).toBe("write /tmp/a.md (3 lines)");
  });

  it("formats edit as edit path", () => {
    expect(
      formatToolCall("edit", { file_path: "/tmp/foo.ts" }, bareTheme),
    ).toBe("edit /tmp/foo.ts");
  });

  it("formats grep as grep /pattern/ in path", () => {
    expect(
      formatToolCall("grep", { pattern: "foo", path: "src" }, bareTheme),
    ).toBe("grep /foo/ in src");
  });

  it("formats find as find pattern in path", () => {
    expect(
      formatToolCall("find", { pattern: "*.ts", path: "src" }, bareTheme),
    ).toBe("find *.ts in src");
  });
