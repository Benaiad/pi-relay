...
describe("pi-relay extension entry", () => {
	it("registers relay and replay tools with renderCall and renderResult", () => {
		...
		const stubApi = {
			registerTool(tool: { name: string; renderCall?: unknown; renderResult?: unknown; parameters: unknown }) {
				registered.push({
					name: tool.name,
	...
	it("the registered tool has a TypeBox parameter schema", () => {
		...
		expect((captured as unknown as { parameters: { type?: string } }).parameters.type).toBe("object");
	});
});
