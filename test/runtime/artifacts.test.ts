...
import { ArtifactStore, formatContractViolation } from "../../src/runtime/artifacts.js";
...
describe("ArtifactStore", () => {
	...
	it("rejects writes from a step that is not the declared writer", () => {
		const store = buildStore();
		const result = store.commit(StepId("second"), new Map([[ArtifactId("a"), 1]]));
	...
	it("rejects writes to unknown artifacts", () => {
		const store = buildStore();
		const result = store.commit(StepId("first"), new Map([[ArtifactId("nope"), 1]]));
		expect(isErr(result)).toBe(true);
		if (!isErr(result)) return;
	...
	it("rejects non-serializable values with a shape mismatch", () => {
		...
		cyclic.self = cyclic;
		const result = store.commit(StepId("first"), new Map([[ArtifactId("a"), cyclic]]));
		expect(isErr(result)).toBe(true);
		if (!isErr(result)) return;
	...
	it("preserves committed artifacts across snapshots", () => {
		const store = buildStore();
		store.commit(StepId("first"), new Map([[ArtifactId("a"), { value: "world" }]]));
	...
	it("record_list shape rejects element missing a field", () => {
		...
		if (!isOk(result)) throw new Error("compile should succeed");
		const store = new ArtifactStore(result.value, () => 1);
		const commit = store.commit(StepId("s"), new Map([[ArtifactId("a"), [{ f: "ok", g: "ok" }, { f: "ok" }]]]));
		expect(isErr(commit)).toBe(true);
		if (isErr(commit)) {
