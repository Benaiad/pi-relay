...
class ScriptedActorEngine implements ActorEngine {
  ...
  callCounts(): ReadonlyMap<string, number> {
    ...
    for (const c of this.calls)
      counts.set(c.stepId, (counts.get(c.stepId) ?? 0) + 1);
    return counts;
  }

  priorAttemptCountsFor(stepKey: string): readonly number[] {
    return this.calls
      .filter((c) => c.stepId === stepKey)
      .map((c) => c.priorAttemptCount);
  }
}
...
const buildScheduler = (
  ...
  if (!isOk(compiled))
    throw new Error("compile must succeed for scheduler tests");
...
describe("Scheduler — happy paths", () => {
  ...
  it("routes a verify_command step to its pass edge and continues", async () => {
    ...
    expect(report.outcome).toBe("success");
    expect(
      report.steps.find((s) => unwrap(s.stepId) === "verify")!.lastRoute,
    ).toEqual(RouteId("pass"));
  });
    ...
    const engine = new ScriptedActorEngine(
      new Map([
        [
          "first",
          [
            noCompletion("first try"),
            noCompletion("second try"),
            completed("next", { note: { ok: 1 } }),
          ],
        ],
      ]),
    );
...
describe("Scheduler — audit replay", () => {
  it("replaying the captured audit yields an identical final state", async () => {
    ...
    expect(replayed.committedArtifacts.map(unwrap)).toEqual(
      finalState.committedArtifacts.map(unwrap),
    );
...
describe("Scheduler — terminal routes", () => {
  ...
  it("produces a chronological attempt timeline for a review/fix loop", async () => {
    ...
    const order = timeline.map(
      (entry) => `${unwrap(entry.stepId)}#${entry.attempt.attemptNumber}`,
    );
    ...
    const reviewEntries = timeline.filter(
      (entry) => unwrap(entry.stepId) === "review",
    );
  ...
  it("exposes per-attempt history in the run report for re-entered steps", async () => {
    ...
    const reviewSummary = report.steps.find(
      (s) => unwrap(s.stepId) === "review",
    )!;
  ...
  it("halts when an action step exceeds its maxRuns cap", async () => {
    ...
    expect(
      (runCounts.get("a") ?? 0) + (runCounts.get("b") ?? 0),
  ...
  it("marks unreached branches as skipped after run_finished fires", async () => {
    ...
    const engine = new ScriptedActorEngine(
      new Map([["decide", [completed("good")]]]),
    );
    const { scheduler } = buildScheduler(plan, engine);
    const report = await scheduler.run();
    expect(report.outcome).toBe("success");
    const failureBranch = report.steps.find(
      (s) => unwrap(s.stepId) === "failure_terminal",
    )!;
