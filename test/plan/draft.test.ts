...
const validPlan: PlanDraftDoc = {
	...
	artifacts: [
		...
		{ id: "implementation-notes", description: "Notes from the implementer", shape: { kind: "untyped_json" } },
	],
	steps: [
		{
			kind: "action",
		...
		},
		{
			...
			routes: [{ route: "done", to: "run-tests" }],
			retry: { maxAttempts: 2 },
		},
		{
			kind: "check",
...
describe("PlanDraftSchema", () => {
	...
	it("rejects an unknown step kind", () => {
		const bad = {
			...validPlan,
			steps: [
				{ kind: "nonsense", id: "weird", actor: "worker", instruction: "hi", reads: [], writes: [], routes: [] },
				...validPlan.steps.slice(1),
			],
		};
		expect(Value.Check(PlanDraftSchema, bad)).toBe(false);
		...
		const bad: unknown = {
			...
			steps: [
				...
				validPlan.steps[1],
				{
					kind: "check",
					id: "run-tests",
					check: { kind: "not_a_real_check", path: "/tmp/x" },
					onPass: "done",
					onFail: "failed",
				},
	...
	it("accepts the minimal shape: one action step plus a terminal", () => {
		const minimal: PlanDraftDoc = {
			...
			steps: [
				...
				{ kind: "terminal", id: "end", outcome: "success", summary: "Greeted." },
			],
			entryStep: "greet",
		};
