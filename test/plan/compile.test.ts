...
describe("compile", () => {
	...
	it("rejects a route targeting a non-existent step", () => {
		...
		const bad: PlanDraftDoc = {
			...basicPlan,
			steps: [{ ...planStep, routes: [{ route: "next", to: "nowhere" }] }, ...basicPlan.steps.slice(1)],
		};
		const result = compile(bad, defaultActors, fixedIdOptions);
		if (!isErr(result)) throw new Error("expected error");
	...
	it("rejects multiple writers for one artifact", () => {
		...
		if (result.error.kind === "multiple_artifact_writers") {
			expect(unwrap(result.error.artifactId)).toBe("requirements");
			expect(result.error.writers.map(unwrap)).toEqual(["plan", "implement"]);
		}
	});
	...
	it("rejects an artifact with no writer", () => {
		const bad: PlanDraftDoc = {
			...
			artifacts: [
				...basicPlan.artifacts,
				{ id: "unused", description: "never produced", shape: { kind: "untyped_json" } },
			],
		};
		const result = compile(bad, defaultActors, fixedIdOptions);
		if (!isErr(result)) throw new Error("expected error");
	...
	it("rejects duplicate artifact declarations", () => {
		const bad: PlanDraftDoc = {
			...
			artifacts: [
				...basicPlan.artifacts,
				{ id: "requirements", description: "dup", shape: { kind: "untyped_json" } },
			],
		};
		const result = compile(bad, defaultActors, fixedIdOptions);
		if (!isErr(result)) throw new Error("expected error");
	...
	it("rejects context policy other than fresh_per_run", () => {
		...
		const bad: PlanDraftDoc = {
			...basicPlan,
			steps: [{ ...planStep, contextPolicy: "persist_per_actor" as const }, ...basicPlan.steps.slice(1)],
		};
		const result = compile(bad, defaultActors, fixedIdOptions);
		if (!isErr(result)) throw new Error("expected error");
	...
	it("compiles a check-step plan with pass and fail routes", () => {
		const plan: PlanDraftDoc = {
			task: "Run tests and branch on outcome.",
			artifacts: [{ id: "spec", description: "spec", shape: { kind: "untyped_json" } }],
			steps: [
				{
					kind: "action",
					id: "write",
				...
				},
				{
					kind: "check",
					id: "verify",
					check: { kind: "file_exists", path: "/tmp/does-not-matter" },
					onPass: "ok",
					onFail: "broken",
				},
	...
	it("rejects a check step with an unknown onFail target", () => {
		const plan: PlanDraftDoc = {
			task: "bad check",
			artifacts: [],
			steps: [
				{
					kind: "check",
					id: "verify",
					check: { kind: "file_exists", path: "/tmp/x" },
					onPass: "done",
					onFail: "ghost",
				},
