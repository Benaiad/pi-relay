...
import { filterActors, filterPlans, loadRelayConfig, saveRelayConfig } from "../src/config.js";
...
describe("loadRelayConfig", () => {
	...
	it("loads disabled actors and plans from config file", async () => {
		await writeFile(
			...
			JSON.stringify({ disabledActors: ["critic", "judge"], disabledPlans: ["debate"] }),
		);
		const config = loadRelayConfig(agentDir);
	...
	it("returns empty sets for malformed JSON", async () => {
		await writeFile(path.join(agentDir, "pi-relay", "config.json"), "not json{{{");
		const config = loadRelayConfig(agentDir);
		expect(config.disabledActors.size).toBe(0);
		expect(config.disabledPlans.size).toBe(0);
	});
	...
	it("returns empty sets when JSON is not an object", async () => {
		await writeFile(path.join(agentDir, "pi-relay", "config.json"), '"just a string"');
		const config = loadRelayConfig(agentDir);
		expect(config.disabledActors.size).toBe(0);
		expect(config.disabledPlans.size).toBe(0);
	});
	...
	it("handles missing disabledPlans key", async () => {
		await writeFile(path.join(agentDir, "pi-relay", "config.json"), JSON.stringify({ disabledActors: ["worker"] }));
		const config = loadRelayConfig(agentDir);
	...
	it("handles missing disabledActors key", async () => {
		await writeFile(path.join(agentDir, "pi-relay", "config.json"), JSON.stringify({ disabledPlans: ["debate"] }));
		const config = loadRelayConfig(agentDir);
		expect(config.disabledActors.size).toBe(0);
		expect(config.disabledPlans).toEqual(new Set(["debate"]));
	});
...
describe("saveRelayConfig", () => {
	...
	it("creates the directory and writes the config file", () => {
		...
		const raw = readFileSync(path.join(agentDir, "pi-relay", "config.json"), "utf-8");
...
describe("filterActors", () => {
	it("returns all actors when nothing is disabled", () => {
		...
		const config = { disabledActors: new Set<string>(), disabledPlans: new Set<string>() };
	...
	it("excludes disabled actors", () => {
		...
		const config = { disabledActors: new Set(["critic"]), disabledPlans: new Set<string>() };
	...
	it("handles stale names in config gracefully", () => {
		...
		const config = { disabledActors: new Set(["ghost"]), disabledPlans: new Set<string>() };
		expect(filterActors(actors, config)).toEqual(actors);
	});
});
...
describe("filterPlans", () => {
	...
	it("excludes disabled plans", () => {
		...
		const config = { disabledActors: new Set<string>(), disabledPlans: new Set(["debate"]) };
