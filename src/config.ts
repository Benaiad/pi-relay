...
export const loadRelayConfig = (agentDir?: string): RelayConfig => {
	...
	try {
		...
		console.error(`[relay] Malformed config at ${configPath}, treating as empty`);
		return EMPTY_CONFIG;
	}
...
export const saveRelayConfig = (config: RelayConfig, agentDir?: string): void => {
	const configPath = join(agentDir ?? getAgentDir(), CONFIG_PATH);
...
export const filterActors = (actors: readonly ActorConfig[], config: RelayConfig): readonly ActorConfig[] =>
	actors.filter((a) => !config.disabledActors.has(a.name));

export const filterPlans = (templates: readonly PlanTemplate[], config: RelayConfig): readonly PlanTemplate[] =>
	templates.filter((t) => !config.disabledPlans.has(t.name));

export const configsEqual = (a: RelayConfig, b: RelayConfig): boolean =>
	setsEqual(a.disabledActors, b.disabledActors) && setsEqual(a.disabledPlans, b.disabledPlans);
