...
export const discoverActors = (
  ...
  const bundledActors = options.bundledDir
    ? loadActorsFromDir(options.bundledDir, "bundled")
    : [];
  const userActors = wantsUser ? loadActorsFromDir(userDir, "user") : [];
  const projectActors =
    wantsProject && projectDir ? loadActorsFromDir(projectDir, "project") : [];
 ...
 */
export const actorRegistryFromDiscovery = (
...
const parseActorFile = (
  ...
  const tools =
    typeof frontmatter.tools === "string"
      ? splitCommaList(frontmatter.tools)
      : undefined;
  const model =
