...
const runAction = async (request: ActionRequest): Promise<ActionOutcome> => {
  ...
  try {
    ...
    if (actor.tools && actor.tools.length > 0)
      args.push("--tools", actor.tools.join(","));
    ...
    if (!parsed.ok) {
      const tail =
        truncate(finalText.trim(), 600) || "(actor produced no text output)";
      return {
        kind: "no_completion",
...
const buildTaskPrompt = (
  ...
  if (priorAttempts.length > 0) {
    ...
    if (skipped > 0) {
      history.push(
        `_(${skipped} earlier attempt${skipped === 1 ? "" : "s"} omitted)_`,
      );
    }
    for (const attempt of capped) {
      const tools =
        attempt.toolsCalled.length > 0
          ? `tools used: ${attempt.toolsCalled.join(", ")}`
          : "no tools used";
      const narrationLine =
        attempt.narration.length > 0
          ? `  "${attempt.narration}"`
          : "  (no narration)";
      history.push(
        `### Attempt ${attempt.attemptNumber} → ${attempt.outcomeLabel}`,
        `  ${tools}`,
        narrationLine,
      );
    }
...
const writePromptToTempFile = async (
  ...
  const dir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "pi-relay-actor-"),
  );
