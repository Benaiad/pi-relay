...
import { buildCompletionInstruction, formatShapeHint, parseCompletion } from "./complete-step.js";
...
const runAction = async (request: ActionRequest): Promise<ActionOutcome> => {
	...
	try {
		...
		if (!step.routes.has(routeId)) {
			return {
				kind: "no_completion",
				reason: `route '${parsed.value.route}' is not one of the allowed routes: ${[...step.routes.keys()]
					.map(unwrap)
					.join(", ")}`,
				usage: result.usage,
				transcript: result.transcript,
