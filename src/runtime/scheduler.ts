...
const describeVerifyStep = (
  step: VerifyCommandStep | VerifyFilesExistStep,
): string => {
  switch (step.kind) {
    case "verify_command":
...
export class Scheduler {
  ...
  constructor(config: SchedulerConfig) {
    ...
    this.artifactStore =
      config.artifactStore ?? new ArtifactStore(this.program, this.clock);
  ...
  async run(): Promise<RunReport> {
    ...
    while (this.readyQueue.length > 0) {
      ...
      if (!step) {
        this.finishWith(
          "failed",
          `scheduler picked unknown step '${unwrap(nextId)}'`,
        );
    ...
    if (this.state.phase === "running" || this.state.phase === "pending") {
      this.finishWith(
        "incomplete",
        `ready queue drained without reaching a terminal step`,
      );
    }
  ...
  private async executeAction(step: ActionStep): Promise<void> {
    ...
    if (!actor) {
      ...
      this.followFailureOrTerminal(
        step,
        `actor '${unwrap(step.actor)}' not found`,
      );
      return;
    }
    ...
    const priorAttemptHistory =
      buildAttemptHistories(this.audit.entries()).get(step.id) ?? [];
    const priorAttempts = priorAttemptHistory.map(
      (entry): PriorAttempt => summarizePriorAttempt(entry),
    );
  ...
  private handleActionCompleted(
    ...
    const commitResult = this.artifactStore.commit(
      step.id,
      writes,
      currentAttempt,
    );
  ...
  private async executeVerifyFilesExist(
    step: VerifyFilesExistStep,
  ): Promise<void> {
    const attempt = (this.state.steps.get(step.id)?.attempts ?? 0) + 1;
    this.emit({
      kind: "step_started",
  ...
  private executeTerminal(step: TerminalStep): void {
    ...
    this.finishWith(
      step.outcome === "success" ? "succeeded" : "failed",
      step.summary,
    );
  }
  ...
  private applyRetryOrFail(step: ActionStep, reason: string): void {
    ...
    this.followFailureOrTerminal(
      step,
      `${SYNTHETIC_FAILURE_REASON_PREFIX}${reason}`,
    );
  }
