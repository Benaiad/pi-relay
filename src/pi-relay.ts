...
export default function (pi: ExtensionAPI): void {
	...
	const registerTools = (actors: readonly ActorConfig[], templates: readonly PlanTemplate[]): void => {
		pi.registerTool<typeof PlanDraftSchema, RelayDetails, RelayRenderState>({
			name: "relay",
			label: "Relay",
			description: buildToolDescription(actors),
		...
		registerReplayTool(pi, templates, {
			actorsDir: bundledActorsDir,
			plansDir: bundledPlansDir,
		});
	};

	registerTools(enabledActors, enabledTemplates);
...
export const buildToolDescription = (actors: readonly ActorConfig[]): string => {
	...
	return [
		...
		'  routes: { "<route_name>": "<target_step_name>", ... } — at least one entry required.',
		"    The actor must emit exactly one route name on completion. This is how control flow works:",
		'    e.g. { "done": "verify", "needs_fix": "retry" }',
		"  reads/writes: (optional) artifact names this step reads/produces",
		"  max_runs: (optional) max activations, defaults to 10",
		"",
