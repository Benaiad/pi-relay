...
export const discoverPlanTemplates = (
  ...
  const wantsProject =
    (scope === "project" || scope === "both") && projectDir !== null;

  const bundledTemplates = options.bundledDir
    ? loadTemplatesFromDir(options.bundledDir, "bundled", warnings)
    : [];
  const userTemplates = wantsUser
    ? loadTemplatesFromDir(userDir, "user", warnings)
    : [];
  const projectTemplates =
    wantsProject && projectDir
      ? loadTemplatesFromDir(projectDir, "project", warnings)
      : [];
  ...
  for (const entry of raw) {
    if (typeof entry !== "object" || entry === null) {
      warnings.push({
        templateName,
        message:
          "Each parameter must be an object with 'name' and 'description'",
        filePath,
      });
      return null;
    }
