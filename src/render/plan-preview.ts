...
const formatPlan = (
  ...
  if (plan.successCriteria) {
    lines.push(
      `  ${theme.fg("muted", `success when: ${plan.successCriteria}`)}`,
    );
  }
...
const buildCountSummary = (plan: PlanDraftDoc): string => {
  ...
  for (const step of plan.steps)
    if (step.kind === "action") actorNames.add(step.actor);
  const agentList =
    actorNames.size > 0 ? Array.from(actorNames).join(", ") : "none";
...
const buildVerifyCommandBlock = (
  ...
  const timeoutSuffix = step.timeoutMs
    ? `  (timeout ${Math.round(step.timeoutMs / 1000)}s)`
    : "";
