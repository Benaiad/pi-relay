...
const wrapReason = (
  ...
  const limited =
    text.length > ERROR_REASON_TRUNCATE
      ? `${text.slice(0, ERROR_REASON_TRUNCATE)}…`
      : text;
...
const formatCollapsed = (
  ...
  if (
    state.phase !== "running" &&
    state.phase !== "pending" &&
    hasExpandedDetail(state)
  ) {
    const last = lines[lines.length - 1] ?? "";
    lines[lines.length - 1] =
      `${last}  ${theme.fg("muted", `(${keyHint("app.tools.expand", "to expand")})`)}`;
  }
...
const buildProgressDetail = (
  ...
  const skippedSuffix =
    skipped > 0 ? theme.fg("muted", ` · ${skipped} not reached`) : "";
  ...
  switch (state.phase) {
    ...
    case "running": {
      ...
      const activity = describeActiveStep(
        active.step,
        active.runtime,
        theme,
        checkOutput?.get(active.stepId),
      );
    ...
    case "aborted":
      return (
        theme.fg("warning", `aborted after ${done} of ${executed} steps`) +
        skippedSuffix
      );
    ...
    case "incomplete":
      return (
        theme.fg(
          "warning",
          `stopped before finishing (${done}/${executed} done without reaching an end state)`,
        ) + skippedSuffix
      );
  }
};
...
const renderExpanded = (
  ...
  if (skippedIds.length > 0) {
    ...
    const more =
      skippedIds.length > 6 ? ` + ${skippedIds.length - 6} more` : "";
    container.addChild(
      new Text(
...
const appendAttemptBlock = (
  ...
  const actorTag =
    step.kind === "action"
      ? `  ${theme.fg("muted", `— ${unwrap(step.actor)}`)}`
      : "";
  const retryTag =
  ...
  if (step.kind === "action") {
    ...
    container.addChild(
      new Text(
        `  ${theme.fg("toolOutput", describeVerifyStepInline(step))}`,
        0,
        0,
      ),
    );
    ...
    container.addChild(
      new Text(`  ${theme.fg("toolOutput", step.summary)}`, 0, 0),
    );
  }
  ...
  if (attempt.outcome === "completed" && attempt.route) {
    ...
    if (!GENERIC_ROUTE_NAMES.has(routeName.toLowerCase())) {
      container.addChild(
        new Text(`  ${theme.fg("success", `→ ${routeName}`)}`, 0, 0),
      );
    }
  } else if (
    ...
    container.addChild(
      new Text(
        `  ${theme.fg("error", `Failed: ${attempt.reason ?? "no reason"}`)}`,
        0,
        0,
      ),
    );
...
const renderActionTranscript = (
  ...
  if (transcript.length > 0) {
    ...
    if (
      last?.kind === "text" &&
      stripCompletionTag(last.text).trim().length > 0
    ) {
  ...
  const droppedToolCalls = Math.max(
    0,
    toolCallPositions.length - EXPANDED_TRANSCRIPT_LIMIT,
  );
  const droppedToolCallIndices = new Set(
    toolCallPositions.slice(0, droppedToolCalls),
  );
  if (droppedToolCalls > 0) {
    container.addChild(
      new Text(
        `  ${theme.fg("muted", `(${droppedToolCalls} earlier tool calls)`)}`,
        0,
        0,
      ),
    );
  ...
  for (let i = 0; i < transcript.length; i += 1) {
    ...
    if (item.kind === "tool_call") {
      container.addChild(
        new Text(
          `  ${theme.fg("muted", "→ ")}${formatToolCall(item.toolName, item.args, theme)}`,
          0,
          0,
        ),
      );
...
const appendCheckOutputPreview = (
  container: Container,
  output: string,
  theme: Theme,
): void => {
  ...
  switch (outcome) {
    ...
    case "terminal": {
      const terminalOutcome: TerminalOutcome =
        step.kind === "terminal" ? step.outcome : "success";
...
const findActiveStep = (
  state: RelayRunState,
): { stepId: StepId; step: Step; runtime: StepRuntimeState } | null => {
  for (const stepId of state.program.stepOrder) {
    const runtime = state.steps.get(stepId);
...
const findFailedStep = (
  state: RelayRunState,
): { stepId: StepId; runtime: StepRuntimeState } | null => {
  for (const stepId of state.program.stepOrder) {
    const runtime = state.steps.get(stepId);
    if (!runtime) continue;
...
const countStatus = (
  state: RelayRunState,
  statuses: readonly StepRuntimeState["status"][],
): number => {
  let n = 0;
...
const hasExpandedDetail = (state: RelayRunState): boolean => {
  for (const runtime of state.steps.values()) {
    ...
    if (runtime.status === "failed" || runtime.status === "succeeded")
      return true;
  }
  return false;
};
